    ...
    <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
      {visible.length === 0 && !isProcessing && (
        <div className="flex items-center justify-center h-full text-gray-500">
          <div className="text-center space-y-2"><p className="text-lg">Start a conversation</p><p className="text-sm">Speak or type a message</p></div>
        </div>
      )}
      {visible.map((msg) => (
          ...
          <div className={'max-w-[85%] rounded-2xl px-4 py-3 ' + (msg.role === 'user' ? 'bg-blue-600 text-white rounded-br-md' : 'bg-gray-800 text-gray-100 rounded-bl-md')}>
            {msg.images && msg.images.length > 0 && (
              <div className="flex gap-1 mb-2 flex-wrap">
                {msg.images.map((img, i) => <img key={i} src={'data:image/jpeg;base64,' + img} alt="frame" className="w-16 h-12 object-cover rounded-lg border border-gray-600" />)}
              </div>
            )}
