...
interface Props { cameraEnabled: boolean; micEnabled: boolean; isRecording: boolean; isSpeaking: boolean; isProcessing: boolean; currentMode: CostMode; onToggleCamera: () => void; onToggleMic: () => void; onToggleSettings: () => void; onToggleCostMode: () => void; }
const modeLabels: Record<CostMode, string> = { off: 'Off', balanced: '均衡', aggressive: '省电' };
export const StatusBar: React.FC<Props> = ({ cameraEnabled, micEnabled, isRecording, isSpeaking, isProcessing, currentMode, onToggleCamera, onToggleMic, onToggleSettings, onToggleCostMode }) => (
  <div className="flex items-center justify-between px-4 py-2.5 bg-gray-900 border-t border-gray-800">
    <div className="flex items-center gap-3">
      ...
      <button onClick={onToggleMic} className={'flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-medium transition-colors ' + (micEnabled ? (isRecording ? 'bg-red-900/40 text-red-400 animate-pulse' : 'bg-green-900/40 text-green-400') : 'bg-gray-800 text-gray-500')}>
        ...
        {isRecording ? '录音中' : micEnabled ? '麦克风' : '麦克风关'}
      </button>
      {isSpeaking && <span className="flex items-center gap-1 text-xs text-blue-400"><span className="w-1.5 h-1.5 bg-blue-400 rounded-full animate-pulse" />AI 说话中</span>}
      {isProcessing && <span className="text-xs text-yellow-400">思考中...</span>}
    </div>
    <div className="flex items-center gap-2">
      <button onClick={onToggleCostMode} className="px-2 py-1 rounded-lg text-xs font-medium bg-gray-800 text-gray-400 hover:bg-gray-700">{modeLabels[currentMode]}</button>
