...
export class ErrorBoundary extends React.Component<Props, State> {
  ...
  render() {
    if (this.state.hasError) {
      ...
      const friendly = msg.includes('API key')||msg.includes('API Key')?'API Key is not configured correctly. Check your .env file.'
        : msg.includes('network')||msg.includes('Network')?'Network connection failed. Please check and retry.'
        : msg.length>120?msg.slice(0,120)+'...':msg||'Unknown error';
      return (<div className="flex items-center justify-center min-h-screen bg-gray-950 p-8">
        <div className="bg-red-900/20 border border-red-800 rounded-xl p-6 max-w-md text-center space-y-4">
          ...
          <p className="text-sm text-red-200/70">{friendly}</p>
